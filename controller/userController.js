...
const userController = {
 ...
 saveStudentProfile: (req, res) => {
  ...
  const { name, email, additionalData } = req.body; // Extract data from the request body
  ...
  const updatedData = { name, email, additionalData };
  ...
  userModel.updateUserProfile(userId, updatedData, (err, result) => {
      ...
      // Check if the profile update affected any rows
...
getStudentProfile: async (req, res) => {
  try {
    ...
    const student = await StudentProfile.findByUserId(userId);
    if (!student) {
...
updateStudentProfile: async (req, res) => {
  try {
    ...
    const { lastname, firstname, degree_program, year_level, phone_number, gmail } = req.body;

    const updated = await StudentProfile.updateByUserId(userId, {
      lastname,
      firstname,
      degree_program,
      year_level,
