...
const adminController = {
  ...
  fetchPosts: (req, res) => {
    ...
    rejectStudent: (req, res) => {
        const studentId = req.params.studentId; // Get studentId from route parameters
        const reason = req.body.reason; // Get rejection reason from request body
        ...
        // Step 1: Check the student's current status
            ...
            if (results.length === 0) {
                adminModel.insertStatus(studentId, (insertErr) => {
                    ...
                    // Step 3: Update the status to 'rejected' and insert the reason
                    adminModel.updateStatusToRejectedWithReason(studentId, reason, (updateErr) => {
                        if (updateErr) {
            ...
            // Step 5: Update the status to 'rejected' and add the reason
            adminModel.updateStatusToRejectedWithReason(studentId, reason, (updateErr) => {
                if (updateErr) {
                    console.error('Error updating status to rejected:', updateErr);
                    return res.status(500).json({ success: false, message: 'Error rejecting student.' });
